...
public class AutoScrollViewPager extends ViewPager {
	...
	// y2 = ev.getY();
	// }
	// boolean disx = Math.abs(x1 - x2) < 5;
	// boolean disy = Math.abs(y1 - y2) > 20;
	// if(disx && disy){
	// return true;
	// }
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		boolean disx = Math.abs(x1 - x2) < 200;
		boolean disy = Math.abs(y1 - y2) > 50;
		if (disx && disy) {
			return true;
		}
		...
		if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT
			...
			if (ev.getAction() == MotionEvent.ACTION_UP
					&& ((currentItem == 0 && downX < touchX) || (currentItem == pageCount - 1 && downX > touchX))) {
				if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT) {
