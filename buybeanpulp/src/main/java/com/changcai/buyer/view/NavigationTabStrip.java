...
public class NavigationTabStrip extends View implements ViewPager.OnPageChangeListener {
    ...
    /**
     * 设置indicator的颜色 并请求绘制
     * @param color
     */
    public void setStripColor(final int color) {
        mStripPaint.setColor(color);
        /**
         * postInvalidate(); 与invalidate()方法区别就是，postInvalidate()方法可以在UI线程执行，也可以在工作线程执行而invalidate()只能在UI线程操作。但是从重绘速率讲：invalidate()效率高
         */
        postInvalidate();
    }
    ...
    protected void onDraw(final Canvas canvas) {
        // Set bound of strip
        mStripBounds.set(
                mStripLeft - (mStripType == StripType.POINT ? mStripWeight * 0.5F +10f : 0.0F),
                mStripGravity == StripGravity.BOTTOM ? mBounds.height() - mStripWeight : 0.0F,
                mStripRight - (mStripType == StripType.POINT ? mStripWeight * 0.5F-10f : 0.0F),
