...
/**
 * Created by liuxingwei on 2017/3/30.
 */
...
public class IndicatorView extends View {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.IndicatorView_iv_IndicatorColor) {
                ...
                mIndicatorTextSize = a.getDimensionPixelSize(attr, AndroidUtil.sp2px(getContext(), DEFAULT_INDICATOR_TEXT_SIZE_SP));
                ...
                mIndicatorHeight = a.getDimensionPixelSize(attr, AndroidUtil.dip2px(getContext(), DEFAULT_INDICATOR_HEIGHT_DP));
            } else if (attr == R.styleable.IndicatorView_iv_IndicatorTextGap) {
                mIndicatorGap = a.getDimensionPixelSize(attr, AndroidUtil.dip2px(getContext(), DEFAULT_INDICATOR_GAP_DP));
            } else if (attr == R.styleable.IndicatorView_iv_IndicatorLengthExtra) {
                mIndicatorExtra = a.getDimensionPixelSize(attr, AndroidUtil.dip2px(getContext(), DEFAULT_INDICATOR_EXTRA_DP));
    ...
    private void init() {
        ...
        if (mIndicatorTextSize == 0)
            mIndicatorTextSize = AndroidUtil.sp2px(getContext(), DEFAULT_INDICATOR_TEXT_SIZE_SP);

        if (mIndicatorHeight == 0)
            mIndicatorHeight = AndroidUtil.dip2px(getContext(), DEFAULT_INDICATOR_HEIGHT_DP);

        if (mIndicatorGap == 0)
            mIndicatorGap = AndroidUtil.dip2px(getContext(), DEFAULT_INDICATOR_GAP_DP);
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (checkViewPagerOnPageChangeListener(mViewPager)) {
                    ...
                    try {
                        ...
                        LogUtil.d(IndicatorView.TAG, e.getMessage());
                        return false;
                    }
