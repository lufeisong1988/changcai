...
import com.changcai.buyer.im.common.util.sys.SysInfoUtil;
...
public class WelcomeActivity extends UI {
    ...
    private void onIntent() {
        ...
        if (TextUtils.isEmpty(DemoCache.getAccount())) {
            ...
            if (intent != null) {
                if (intent.hasExtra(NimIntent.EXTRA_NOTIFY_CONTENT)) {//lufeisong 拦截点击信息通知栏
                    LogUtil.d("WelocomActivity","EXTRA_NOTIFY_CONTENT");
                    parseNotifyIntent(intent);
                    return;
                } else if (intent.hasExtra(Extras.EXTRA_JUMP_P2P) || intent.hasExtra(AVChatActivity.INTENT_ACTION_AVCHAT)) {
                    LogUtil.d("WelocomActivity","EXTRA_JUMP_P2P || INTENT_ACTION_AVCHAT");
    ...
    private void parseNotifyIntent(Intent intent) {
        /**
         * lufeisong 叠发信息n条，这里只会获取最近一条
         * 所以排除messages为null和信息数大于一条的情况
         */
